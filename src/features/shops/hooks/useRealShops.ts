...
export function useRealShops(): UseRealShopsReturnType {
  ...
  const fetchShops = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        "Error",
        `Failed to fetch shops: ${formattedError.message}. Please try again later or contact support if the issue persists.`
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchShopById = useCallback(async (id: string): Promise<Shop> => {
    ...
    try {
      ...
      toast.error(
        "Error",
        `Failed to fetch shop: ${(err as Error).message}`
      );
  ...
  const createShop = useCallback(async (shop: Omit<Shop, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    try {
      ...
      // Ensure the shop has an address object to avoid rendering errors
      const validatedShop = {
        ...newShop,
      ...
      setShops(prevShops => {
        ...
        const exists = prevShops.some(s => s.id === validatedShop.id);
        if (exists) {
          // Update the existing shop
          return prevShops.map(s => s.id === validatedShop.id ? validatedShop : s);
        } else {
          // Add the new shop
          return [...prevShops, validatedShop];
        }
      });

      toast.success(
        "Success",
        `Shop "${validatedShop.name}" created successfully`
      );
      ...
      toast.error(
        "Error",
        `Failed to create shop: ${(err as Error).message}`
      );
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const updateShop = useCallback(async (id: string, shopData: Partial<Shop>) => {
    ...
    try {
      ...
      // Ensure the shop has all required fields with proper defaults
      const validatedShop = {
        ...updatedShop,
        ...
        // Ensure operatingHours are properly structured if present
        operatingHours: updatedShop.operatingHours || null
      };

      // Update shops list
      setShops(prevShops =>
        prevShops.map(shop =>
          shop.id === id ? validatedShop : shop
        )
      );

      // Update selected shop if it's the one being updated
      if (selectedShop?.id === id) {
        setSelectedShop(validatedShop);
      }

      toast.success(
        "Success",
        `Shop "${validatedShop.name}" updated successfully`
      );
      ...
      toast.error(
        "Error",
        `Failed to update shop: ${(err as Error).message}`
      );
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const deleteShop = useCallback(async (id: string) => {
    ...
    try {
      ...
      toast.error(
        "Error",
        `Failed to delete shop: ${(err as Error).message}`
      );
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const fetchShopInventory = useCallback(async (shopId: string) => {
    ...
    try {
      ...
      toast.error(
        "Error",
        `Failed to fetch inventory: ${(err as Error).message}`
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchStaffAssignments = useCallback(async (shopId: string) => {
    ...
    try {
      ...
      toast.error(
        "Error",
        `Failed to fetch staff assignments: ${(err as Error).message}`
      );
    } finally {
      setIsLoading(false);
    }
