...
const GiftCardService = {
  ...
  redeemGiftCard: async (code: string, amount: number, orderId: string): Promise<{ 
    ...
    const validationResult = await GiftCardService.validateGiftCard(code);
    
    if (!validationResult.valid || !validationResult.giftCard) {
    ...
    const amountToUse = amount > giftCard.balance ? giftCard.balance : amount;
