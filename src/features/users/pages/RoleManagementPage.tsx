...
import { roleTemplateService } from '@/features/users/services';
...
export function RoleManagementPage() {
  ...
  // Fetch roles on mount and handle cleanup on unmount
  useEffect(() => {
  ...
  const handleCreateRole = async () => {
    ...
    try {
      ...
      toast.success("Success", `Role "${newRole.name}" was created successfully.`);
      ...
      toast.error("Error", `Failed to create role: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      if (!cleanupInProgress.current) {
        setIsSubmitting(false);
      }
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      ...
      // Show success message
      toast.success("Success", `Role "${newRole.name}" was updated successfully.`);
      ...
      toast.error("Error", `Failed to update role: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      if (!cleanupInProgress.current) {
        setIsSubmitting(false);
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Show success message
      toast.success("Success", "Role was deleted successfully.");
      ...
      toast.error("Error", `Failed to delete role: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      if (!cleanupInProgress.current) {
        setIsSubmitting(false);
      }
